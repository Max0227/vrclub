...
const Pricing = ({ onBooking }: PricingProps) => {
  const highlights = [
    { label: 'VR 1 час', value: '800 ₽', sub: '1 шлем Oculus Quest 2', color: '#00f5ff', icon: 'ri-vr-glasses-line' },
    { label: 'VR 4 шлема × 1 час', value: '3 200 ₽', sub: 'Совместная игра', color: '#00f5ff', icon: 'ri-group-line' },
    { label: 'MOZA Racing 1 час', value: '1 000 ₽', sub: 'Профессиональный симулятор', color: '#ff006e', icon: 'ri-steering-2-line' },
    { label: 'PlayStation 5 — 1 час', value: '350 ₽', sub: '4K · до 4 игроков', color: '#9b4dff', icon: 'ri-gamepad-line' },
    { label: 'Весь клуб — 1 час', value: '4 750 ₽', sub: 'VR + PS5 + MOZA · до 12 чел', color: '#00f5ff', icon: 'ri-building-2-line' },
    { label: 'Скидка на ДР', value: '−20%', sub: '±3 дня от даты рождения', color: '#ff006e', icon: 'ri-cake-2-line' },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="font-mono-tech text-xs tracking-widest" style={{ color: '#00f5ff' }}>СТОИМОСТЬ</span>
            <span className="tag-line" />
          </div>
          ...
          {highlights.map((h) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-mono-tech text-white/40 leading-tight" style={{ fontSize: '9px' }}>{h.label}</span>
              </div>
              <span className="font-orbitron font-black text-2xl leading-none mb-1" style={{ color: h.color }}>{h.value}</span>
              <span className="font-rajdhani text-white/35 text-xs leading-tight">{h.sub}</span>
              {/* Bottom accent line */}
              <div className="absolute bottom-0 left-0 right-0 h-px rounded-b-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        ...
        <div className="flex flex-col items-center gap-5">
          ...
          >
            ...
            />
            <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(1,0,20,0.92) 0%, rgba(1,0,20,0.6) 50%, rgba(1,0,20,0.92) 100%)' }} />
            {/* Glow top */}
            <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% -20%, rgba(0,245,255,0.1) 0%, transparent 65%)' }} />

            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6 px-8 py-8">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <i className="ri-price-tag-3-line text-sm" style={{ color: '#00f5ff' }} />
                  <span className="font-mono-tech text-xs tracking-widest" style={{ color: '#00f5ff' }}>ПОЛНЫЙ ПРАЙС-ЛИСТ</span>
                </div>
              ...
              <div className="flex flex-col gap-3 flex-shrink-0">
                ...
                <button
                  onClick={() => onBooking()}
                  className="btn-cyber-pink px-7 py-3 text-sm rounded-sm whitespace-nowrap font-orbitron font-bold"
                >
                  Записаться
                </button>
              </div>
